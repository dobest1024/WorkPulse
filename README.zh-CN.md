# 拾光 (WorkPulse)

一款轻量桌面应用，几秒钟记录你的每日工作——写下做了什么、用看板管理任务、用 AI 生成工作报告。

为每一个想轻松回顾"今天干了啥"的打工人而生。

## 功能

**工作日志** — 输入你刚做了什么，按回车，完成。支持 `#标签` 自动分类、全文搜索、CSV/Markdown 导出。

**看板任务** — 在待办 → 进行中 → 已完成之间拖拽任务卡片。有草稿箱存放"以后再说"的想法，支持截止日期和内联编辑。完成任务时自动生成一条工作日志。

**AI 报告** — 选择时间范围，一键生成结构化工作总结。由 Claude API 驱动，支持周报/月报/季报，带历史记录。

**数据统计** — 14 天活动柱状图、GitHub 风格热力图、连续记录天数、任务完成统计。

**快速记录** — 全局快捷键（`Ctrl+Shift+L` 记日志、`Ctrl+Shift+T` 加任务）让你无需切换窗口即可记录。也可从菜单栏托盘图标操作。

**深色模式** — 跟随系统、浅色、深色三种主题，全界面覆盖。

## 技术栈

- **Electron** + **React** + **TypeScript**
- **Vite** (electron-vite) 快速构建
- **SQLite** (better-sqlite3) 本地数据存储
- **Zustand** 状态管理
- **@dnd-kit** 拖拽排序
- **Tailwind CSS** 样式

## 快速开始

```bash
# 安装依赖
npm install

# 开发模式运行
npm run dev

# 生产构建
npm run build

# 打包分发
npm run dist:mac    # macOS (DMG + ZIP, x64 + arm64)
npm run dist:win    # Windows (NSIS 安装包)
npm run dist:linux  # Linux (AppImage)
```

## 快捷键

| 快捷键 | 功能 |
|--------|------|
| `⌘1` / `Ctrl+1` | 切换到工作日志 |
| `⌘2` / `Ctrl+2` | 切换到看板 |
| `⌘3` / `Ctrl+3` | 切换到报告 |
| `⌘4` / `Ctrl+4` | 切换到统计 |
| `⌘,` / `Ctrl+,` | 设置 |
| `Ctrl+Shift+L` | 全局快速记录日志 |
| `Ctrl+Shift+T` | 全局快速添加任务 |

## 数据存储

所有数据存储在本地 SQLite 数据库中（系统应用数据目录）。每日自动备份。无云端、无账号、无遥测。

## 许可证

MIT
